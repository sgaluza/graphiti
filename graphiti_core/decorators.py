...
def handle_multiple_group_ids(func: F) -> F:
    ...
    async def wrapper(self, *args, **kwargs):
        ...
        # Handle FalkorDB: each group_id maps to a separate FalkorDB graph,
        # so we must clone the driver per group_id even for a single group_id
        # when it differs from the current driver._database
        if (
